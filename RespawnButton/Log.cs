...
namespace RespawnButton
{
    internal static class Log
    {
        ...
        {
            const string MOD_NAME_PATH = nameof(RespawnButton) + @"\";

            int modNameLastPathIndex = callerPath.LastIndexOf(MOD_NAME_PATH);
            if (modNameLastPathIndex >= 0)
            {
                callerPath = callerPath.Substring(modNameLastPathIndex + MOD_NAME_PATH.Length);
            }
